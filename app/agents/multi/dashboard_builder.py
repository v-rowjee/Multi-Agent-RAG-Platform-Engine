...
from app.agents.multi.dashboard_charts import DashboardChartBuilder
from app.core.currency import format_currency
from app.schemas.api import DashboardResponse
from app.schemas.dashboard import DashboardLayoutPlan, DashboardSection
from app.services.data.series import value_format_for_measure
...
class MultiDashboardBuilder:
    """Validate layout selections and assemble the final dashboard response."""

    def __init__(self, prepared_dataset: dict[str, Any], dataframe: pd.DataFrame) -> None:
        self.prepared = prepared_dataset
        self.df = dataframe
        self.charts = DashboardChartBuilder(prepared_dataset, dataframe)
    ...
    def validate_plan(
        ...
        return plan.model_copy(
            update={
                ...
                "include_forecast": bool((forecast or {}).get("forecast")),
                "supporting_chart_specs": self.charts.validated_specs(
    ...
    def build(
        ...
        supporting = self.charts.build(plan.supporting_chart_specs)
        anomaly_items = [
            {
                "id": item_id,
