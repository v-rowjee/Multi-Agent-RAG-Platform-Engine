...
async def prepare_retrieval_documents(
    prepared_dataset: dict[str, Any], dataframe: pd.DataFrame | None = None,
    kpi_trend_output: dict[str, Any] | None = None, anomaly_output: dict[str, Any] | None = None,
    forecasting_output: dict[str, Any] | None = None, synthesis_output: dict[str, Any] | None = None,
    ...
    if dataframe is None and (prepared_path := str(prepared.get("prepared_file_path") or "").strip()):
        try: dataframe = pd.read_csv(prepared_path)
        except Exception: dataframe = None
    ...
    for item in kpi.get("kpis", []):
        if item.get("id"): _add(documents, id=_document_id("kpi", item["id"]), document_type="kpi", title=str(item.get("title") or item["id"]), content=f"{item.get('title') or item['id']}: {item.get('value')} ({item.get('aggregation')} of {item.get('measure')}; currency: {currency or 'not identified'}).", source_ids=[str(item["id"])], metadata={"aggregation": item.get("aggregation"), "measure": item.get("measure"), "value": item.get("value"), "raw_value": item.get("raw_value"), "currency": currency})
    for item in kpi.get("trends", []):
        if item.get("id"):
            points = item.get("points") or []; first, last = (points[0], points[-1]) if points else ({}, {})
    ...
    for key, document_type, metadata_key in (("key_insights", "insight", "importance"), ("recommendations", "recommendation", "priority")):
        for item in synthesis.get(key, []):
            if item.get("id"):
                source_ids = [str(ref.get("source_id")) for ref in item.get("evidence", []) if ref.get("source_id")]
                _add(documents, id=_document_id(document_type, item["id"]), document_type=document_type, title=str(item.get("title") or item["id"]), content=str(item.get("description") or ""), source_ids=source_ids, metadata={metadata_key: item.get(metadata_key)})
    existing_ids = {document.id for document in documents}
    for document in dashboard_retrieval_documents(dashboard_output):
        if document.id not in existing_ids: _add(documents, **document.model_dump(mode="json")); existing_ids.add(document.id)
    ...
    if limitations: _add(documents, id="limitations", document_type="limitation", title="Analysis limitations", content=" ".join(str(value) for value in limitations), source_ids=["dataset_summary"], metadata={"count": len(limitations)})
    ...
    return RetrievalPreparationOutput(status="complete" if documents else "partial", documents=documents, limitations=[str(value) for value in limitations], warnings=[str(value) for value in dict.fromkeys([*warnings, *row_warnings])])
